...
wd <- getwd()
data_path <- "/results/CPRDGold_Results/IP/"
...
incidence_estimates_help <- rbind(Inf_Age,Inf_AllandSex,Testneg_Age,Testneg_AllandSex)
...
# infection and test negative cohorts
...
## overall results in test negative  cohort
conditions_testneg <- 
  incidence_estimates_help %>% 
  filter( analysis_interval == "overall", 
          denominator_age_group == "0;150",
...
ggplot(conditions_testneg, aes( x = outcome_cohort_name, y = incidence_100000_pys)) + 
  geom_smooth( method = "lm",
               formula = y ~ poly(x, 1),
               se = FALSE) +
  geom_point( size = 0.5) +
...
#### trends of incidence by year in test negative cohort


conditions_in_testneg_year <- 
  incidence_estimates_help %>% 
  filter( analysis_interval == "years", 
          denominator_strata_cohort_name == "test_negative",
          denominator_age_group == "0;150",
...
ggplot(conditions_in_testneg_year, aes( x = factor(year_index), y = incidence_100000_pys)) + 
  geom_smooth( method = "lm",
               formula = y ~ poly(x, 1),
               se = FALSE) +
  geom_point( size = 0.5) +
...
# Trend of incidence by month in test negative cohort 

conditions_in_testneg_month <- 
  incidence_estimates_help %>% 
  filter( analysis_interval == "months", 
          denominator_strata_cohort_name == "test_negative",
          denominator_age_group == "0;150",
...
ggplot(conditions_in_testneg_month, aes( x = factor(year_month), y = incidence_100000_pys)) + 
  geom_smooth( method = "lm",
               formula = y ~ poly(x, 1),
               se = FALSE) +
  geom_point( size = 0.5) +
...
### by age in test negative cohort

conditions_in_testneg_age <- 
  incidence_estimates_help %>% 
  filter( analysis_interval == "overall", 
          denominator_strata_cohort_name == "test_negative",
          denominator_sex == "Both") %>% 
...
ggplot(conditions_in_testneg_age, aes( x = denominator_age_group, y = incidence_100000_pys)) + 
  geom_smooth( method = "lm",
               formula = y ~ poly(x, 2),
               se = FALSE) +
  geom_point( size = 0.5) +
...
# Incidence by sex in test negative population

conditions_in_testneg_sex <- 
  incidence_estimates_help %>% 
  filter( analysis_interval == "overall", 
          denominator_age_group == "0;150",
          denominator_strata_cohort_name == "test_negative"
...
ggplot(conditions_in_testneg_sex, aes( x = cdm_name, y = incidence_100000_pys, fill = denominator_sex)) + 
  geom_errorbar( aes( ymin = incidence_100000_pys, ymax = incidence_100000_pys_95CI_upper, color = denominator_sex), 
                 position = position_dodge(0.5), width = 0.25) +
  geom_col( position = position_dodge(0.5), width = 0.5) +
  scale_y_continuous( breaks = scales::breaks_extended(5)) +
...
incidence_estimates_wide <- 
  incidence_estimates_help %>% 
  filter( 
    denominator_strata_cohort_name %in% c( "infection", "test_negative")) %>% 
...
children_temp <-
  map_df( setdiff( names_conditions$cohort_name, c("misc","juvenile_arthritis",
                                                   "sle","ra","ibd","me_cfs","pots","t1dm")) %>% set_names,
          cohort_wrap_func,
          interval = "overall",
...
adult_temp <- 
  map_df( setdiff( names_conditions$cohort_name, c("misc","juvenile_arthritis","sle","t1dm")) %>% set_names,
          cohort_wrap_func,
          interval = "overall",
...
elderly_temp <- 
  map_df( setdiff( names_conditions$cohort_name, c("t1dm","misc","me_cfs","juvenile_arthritis","sle","ibd","ra")) %>% set_names,
          cohort_wrap_func,
          interval = "overall",
          age_expre =  c("65;120"),
          sex_expre = "Both",
